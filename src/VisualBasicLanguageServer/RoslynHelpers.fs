module VisualBasicLanguageServer.RoslynHelpers
...
type VisualBasicLspHostServices () =
...
let tryLoadSolutionOnPath
    ...
    async {
        try
            ...
            let msbuildWorkspace = MSBuildWorkspace.Create(VisualBasicLspHostServices())
...
let loadSolutionOnSolutionPathOrDir
    ...
    | None -> async {
        let logMessage: LogMessageParams = {
            Type = MessageType.Info
            Message = sprintf "vb-ls: attempting to find and load solution based on root path (\"%s\").." rootPath
        }
