...
open VisualBasicLanguageServer.Handlers
...
type VisualBasicLspServer(
        lspClient: VisualBasicLspClient,
    ...
    interface IVisualBasicLspServer with
        override __.Dispose() = ()
...
module Server =
    ...
    let startCore settings =
        ...
        let serverCreator client =
            new VisualBasicLspServer(client, settings) :> IVisualBasicLspServer

        let clientCreator = VisualBasicLspClient
